   ...
   # Add Products Button (only for admin users)
      ...
      for row_idx, product in enumerate(products_display):
            ...
            action_delete_button = ctk.CTkButton(
                  ...
                  height=40,
                  cursor="hand2",
                command=lambda name=product_name, user_id=user_id: delete_product_action(name, window, user_id)
                  )
...
def edit_product_action(product_name, window):
    ...
    from components.actions.db.change_image import change_product_image
...
def delete_product_action(product_name, window, user_id):
    ...
    def confirm_delete():
        try:
            ...
            messagebox.showinfo("Success", f"'{product_name}' has been successfully deleted.")
