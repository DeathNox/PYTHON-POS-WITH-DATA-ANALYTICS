...
def fetch_orders(frame, col_widths):
    try:
        ...
        for row_idx, order in enumerate(orders, start=1):
            ...
            # Disable the cancel button if the status is "Completed"
            if order_status == "Completed":
                action_cancel_button.configure(state="disabled")
...
def cancel_order(order_id, frame, col_widths):
    try:
        # 1. Insert the order into tbl_archive
        insert_sql = """
            INSERT INTO tbl_archive
            SELECT * FROM tbl_purchase_order WHERE purchase_order_id = %s
        """
        mycursor.execute(insert_sql, (order_id,))
        db.commit()
        ...
        # 3. Delete the order from tbl_purchase_order
        sql = "DELETE FROM tbl_purchase_order WHERE purchase_order_id = %s"
        mycursor.execute(sql, (order_id,))
        db.commit()

        print(f"Order {order_id} archived and related payments have been canceled.")
...
def orders_container_archive(window, user_id):
    ...
    # fetch data from the database
    fetch_orders_archive(orders_scrollable_frame, col_widths)
...
def fetch_orders_archive(frame, col_widths):
    try:
        sql = "SELECT purchase_order_id, order_date, item_name, quantity, sub_total, order_status FROM tbl_archive ORDER BY purchase_order_id DESC LIMIT 50"
        mycursor.execute(sql)
        orders = mycursor.fetchall()
        ...
        for row_idx, order in enumerate(orders, start=1):
            ...
            status_dropdown = ctk.CTkOptionMenu(
                frame,
                values=status_options,
                command=lambda status, oid=order_id, old_status=order_status: fetch_orders_archive(oid, status, old_status, frame, col_widths),
                text_color="white",
                font=("Inter", 16, "bold"),
                width=col_widths[5],
                fg_color="#6F5E5C"
...
def update_order_status_archive(order_id, new_status, old_status, frame, col_widths):
    """
    Updates the order status in the database and refreshes the metrics if the status is changed.
    """
    try:
        ...
        for widget in frame.winfo_children():
            widget.grid_forget()  
        fetch_orders_archive(frame, col_widths)
