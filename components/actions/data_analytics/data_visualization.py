...
def display_line_chart(frame, period='daily'):
    try:
        ...
        with engine.connect() as db:
            ...
            if period == 'daily':
                df_agg = df.groupby('order_date').sum()
                x_values = df_agg.index.astype(str)  # Convert date index to string for clean x-axis
            elif period == 'weekly':
                df_agg = df.set_index(pd.to_datetime(df['order_date'])).resample('W').sum()
                x_values = df_agg.index
            elif period == 'monthly':
                df_agg = df.set_index(pd.to_datetime(df['order_date'])).resample('M').sum()
                x_values = df_agg.index
            else:
                raise ValueError("Invalid period. Choose from 'daily', 'weekly', or 'monthly'.")

            fig, ax = plt.subplots(figsize=(6, 4))
            ax.plot(x_values, df_agg['total_sale'], marker='o', label='Sales Revenue')
